...
def accept_recording(scope: Scope, external_ref: str, audio: bytes, metadata: dict, idempotency_key: str, *, storage: LocalPrivateStorage | None = None, generation_fence: tuple[str, str, int, str, str, str] | None = None, timeout_seconds: float | None = None) -> dict:
    ...
    if timeout_seconds is not None and (not math.isfinite(timeout_seconds) or timeout_seconds <= 0):
        raise IntakeError("Invalid Exotel intake deadline")
        ...
        with connection_context as connection:
            with connection.transaction():
                ...
                if timeout_seconds is not None:
                    connection.execute("SELECT set_config('statement_timeout', %s, true)", (f"{max(1, int(timeout_seconds * 1000))}ms",))
                if generation_fence is not None:
                    ...
                    memberships = connection.execute(
                        ...
                        (scope.organisation_id, expected_agent_id),
                    ).fetchall()
                    if len(memberships) != 1 or memberships[0][0] != expected_team_id:
                        raise IntakeError("Exotel agent membership is no longer active")
