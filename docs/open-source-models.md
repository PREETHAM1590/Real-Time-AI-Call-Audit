...
Status: transcription and local redaction adapters are implemented; runtime packages are version-pinned optional dependencies, but model artifacts are not provisioned and no quality benchmark has run. This document records the user's requirement that call audio, transcripts and audit prompts be processed by self-hosted models. Keep model files on project-controlled infrastructure and run inference without outbound model API calls. Download and verify model artifacts during controlled provisioning, then pin exact revisions and checksums. Evaluate each candidate on consented, representative contact-centre data before promotion.
...
| Disposition signals | Evaluate the already shortlisted local instruction model/runtime for typed Noul/Choice/Score-like outputs; select a pinned open artifact only after held-out disposition evaluation. Keep the adapter replaceable and taxonomy/rules in versioned JSON. | Per-code precision/recall, confusion pairs, abstention/coverage, calibration error, contradictory-signal rate, schema validity, latency and capacity. Treat generated confidence as uncalibrated until measured against adjudicated data. |
