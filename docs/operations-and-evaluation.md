...
The analyst page at `/analyst` includes a manual upload form for WAV/MP3 recordings. Uploads require an AGENT account with exactly one server-resolved team; QA and administrator accounts cannot use this form. Enter the external reference and language tag. The page blocks files over 250 MiB before sending them and posts multipart data to the existing `POST /v1/calls` endpoint with the CSRF token and an `Idempotency-Key`. If a network failure leaves the result uncertain, retry without changing the form to reuse the same key; editing any field or a successful upload starts a new key. The browser limit is a convenience; backend media validation remains authoritative. Use synthetic recordings until real-data processing and retention policies are approved.
