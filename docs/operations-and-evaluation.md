...
Exotel WSS recording intake persists only a hashed call key, integration/generation identifiers, mapped agent/team IDs and UTC lifecycle timestamps. The scoped `/v1/live-calls` snapshot exposes LIVE, DRAINING, ENDED and INCOMPLETE states, including stale LIVE/DRAINING activity. Live local transcription is disabled by default. With `EXOTEL_LIVE_TRANSCRIPTION=1`, an API-process task consumes a four-second bounded audio queue for local Faster-Whisper and redacts before writing the 15-minute tenant/generation-scoped preview. It reports DISABLED, EMPTY, LIVE or DEGRADED; it evicts the oldest preview rows beyond 500 and marks truncation. Transcription/model/redaction/storage failure and queue overload do not stop valid recording intake. UI utterances remain provisional; post-call transcript is authoritative. No hosted inference or runtime artifact download occurs. Exotel account entitlement, callflow configuration and production connectivity remain unverified.

The snapshot marks LIVE sessions INCOMPLETE after 90 seconds without media activity and DRAINING sessions after 2 hours 1 minute without completion; DRAINING stays visible longer to cover the bounded recording intake. This recovery runs when an authorized snapshot is read. Legacy session rows are backfilled as UNKNOWN and hidden until a new generation is validated. Recording acceptance and the ENDED transition commit in one database transaction; after an ambiguous intake error, the handler confirms the durable call/session before deciding whether to expose INCOMPLETE.
...
**Status: run once, partial result.** Measured on Kaggle's GPU (single Tesla T4), **not target production hardware**. Raw output: [`benchmarks/kaggle_gpu_benchmark/results/20260924T190708Z/benchmark_result.json`](../benchmarks/kaggle_gpu_benchmark/results/20260924T190708Z/benchmark_result.json) (and sibling `.log`), generated 2026-09-24T18:45:33Z, against `config/rubric.v1.json` (sha256 `26fdf17a…`) and `prompts/audit.v1.txt` (sha256 `343a382a…`) as pinned at run time.

- **faster-whisper (`small`, resolved revision `536b0662…`)** — status **OK**. Model load 6.224s; a synthetic 30s silent clip decoded in 1.997s (real-time factor 0.0666, i.e. ~15x faster than real time on a T4); `vad_filter=True` correctly returned 0 segments for silence; detected language `en`; total 17.147s. This measures decode throughput on silence only — no word-error-rate signal, since the fixture has no speech.
- **Audit LLM (`Qwen/Qwen3-8B`)** — status **FAILED**, both engine paths, after 264.905s. `vllm` load failed: `libcudart.so.13: cannot open shared object file: No such file or directory` (CUDA runtime library mismatch against the Kaggle image's installed vLLM build). The `transformers` + 4-bit fallback then also failed: `cannot import name '_maybe_view_chunk_cat' from 'torch._utils'` (an incompatible `transformers`/`torch` pairing pulled by unpinned `pip install`). No audit-LLM throughput, JSON-schema-validity, or GPU-memory number exists yet — the harness's own `pip install vllm` / `pip install transformers accelerate bitsandbytes` calls are unpinned and picked versions incompatible with the Kaggle base image's CUDA/torch install.

This is not a production-readiness claim: it is a single Kaggle run, on synthetic silence, that exercises only half the pipeline. **Fixed, not yet re-run:** `run_benchmark.py.template` no longer `pip install`s `vllm` by default (it only uses vLLM if already importable on the image; `BENCHMARK_TRY_VLLM=1` opts into an explicit, pinned install after separately confirming CUDA compatibility) and no longer installs or upgrades `torch` at all on the `transformers` fallback path, so a failed vLLM attempt can no longer corrupt the fallback's own dependencies the way it did in the run above. Verified by simulated control-flow tests (vLLM absent → no `vllm` install attempted, `transformers`+4-bit path runs against a mocked model and produces a schema-valid result; both engines absent → a clean two-part error) — not by an actual Kaggle GPU run, since none is available from this workspace. The benchmark needs to actually be run again on Kaggle before this row can report a real audit-LLM latency/schema/memory number.
...
The worker emits one structured `worker.stage_outcome` log record for each claimed attempt, with only allowlisted stage, bounded attempt number, outcome and elapsed milliseconds. It does not log job/call/tenant identifiers, exception text, transcript text, or model request/response bodies. `GET /health` is a process liveness check. `GET /ready` checks `SELECT 1` and returns database availability with `model_readiness: unknown`; it does not probe or claim local model readiness. `GET /v1/operations/summary` is ADMIN-only and tenant-scoped; it returns pending job count, oldest pending age capped at 365 days, calls not in READY, and a per-stage breakdown (`pending_by_stage`, stage names bounded to a known allowlist), with no per-call rows or identifiers. The incomplete count reflects pipeline state (and includes failed/needs-review records), not trusted telephony call completeness. The tenant-scoped dashboard (`/dashboard`) renders that per-stage breakdown as a small table, with an explicit unavailable/empty state instead of implying all-clear when the summary fails to load.

A metrics exporter now exists: `GET /metrics` (`app/metrics.py`, called from `app/api.py`) serves Prometheus text exposition format 0.0.4. It is disabled (404) unless `METRICS_TOKEN` is set to at least 32 characters, is a separate secret from OIDC user auth, requires `Authorization: Bearer <token>` checked with `hmac.compare_digest`, and is excluded from the OpenAPI schema. It emits aggregate, cross-tenant series with only bounded enum labels (never a tenant/call/agent/team identifier): `call_audit_jobs{stage,state}`, `call_audit_oldest_pending_job_age_seconds{stage}`, `call_audit_calls{processing_state}`, `call_audit_worker_stage_attempts_total{stage,outcome}` and `call_audit_worker_stage_duration_milliseconds_total{stage,outcome}` (from a new cross-process `worker_stage_counters` table the worker upserts after every claimed attempt, migration `022_worker_stage_counters.sql`), `call_audit_live_sessions{state}`, `call_audit_event_outbox_events`, and `call_audit_database_up`. Any unexpected label value is folded into `UNKNOWN` so a bad row cannot blow up cardinality, and if the underlying database queries fail the scrape still returns 200 with only `call_audit_database_up 0` — the exporter itself never 500s. Recording a counter update (`app.metrics.record_stage_counter`) opens its own short, bounded-timeout connection, never raises, and never blocks or fails job processing; a write failure is logged at WARNING with no identifiers. `deploy/prometheus/alerts.yml` and `deploy/prometheus/README.md` give a proposed, undeployed rule file and scrape config (`bearer_token_file`) built only from these series; the README states explicitly that no Alertmanager receiver or notification path is configured anywhere in this repository, and lists which ops-guide alert signals this exporter cannot yet back (redaction failures, STT failure rate, and GPU/token/spend telemetry, none of which is tracked as a metric).

What still does not exist: no time-series database/storage (Prometheus itself is not deployed, wired into `compose.yaml`, or run in CI), no alert delivery/notification of any kind, no GPU/token/spend telemetry, and no redaction-failure or STT-failure-rate counter (current per-stage/per-outcome counters cannot distinguish those specific causes from other retries within the same stage). Model readiness and inference capacity remain unmeasured until deployment instrumentation and selected local models are configured and load-tested.
...
[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs on every push and pull request: a PostgreSQL 16 service container, the full backend and browser test suite (`test`, `browser-test`, `transcription` and `privacy` extras installed, matching the planned commands), a compile check, and [`scripts/scan_for_secrets.py`](../scripts/scan_for_secrets.py) — a stdlib-only scan for a short list of well-known credential shapes (AWS keys, PEM private-key blocks, GitHub/Slack tokens, and non-placeholder `*_KEY`/`*_TOKEN`/`*_SECRET`/`*_PASSWORD` assignments outside `tests/`/`docs/`/example files). That scanner is a minimal, reviewable safety net, not a maintained scanner like gitleaks or trufflehog; a third-party GitHub Action was deliberately not added here because its exact release could not be verified from this workspace, and referencing an unverified tag/SHA is worse than the smaller in-repo check. Verified by running the equivalent steps locally against a fresh virtualenv and a password-authenticated TCP PostgreSQL 16 instance (matching a GitHub Actions service container, not the socket/trust-auth setup used elsewhere in this repository's own local testing): 262 tests passed. Pinned-model quality/load checks, GPU capacity, and staging restore/deletion drills remain outside this workflow's scope per "Release validation" above — CI proves the code and its deterministic-fake-backed contracts, not model quality or production capacity.

### Exotel AgentStream intake status

The API exposes ADMIN-only tenant-scoped integration and agent-mapping routes under `/v1/exotel-integrations`, plus a synthetic-tested WSS endpoint at `/v1/exotel/stream` that currently requires HTTP Basic credentials. Integration creation returns a generated username/password once; record the password in the approved secret store. **Do not assume the Exotel unidirectional Stream applet sends Basic credentials:** public docs describe WSS Basic authentication for the bidirectional Voicebot applet, while the Stream applet page documents its WSS URL but not authentication headers. Confirm the actual Stream applet handshake with an Exotel account before enabling this adapter; never add an unauthenticated fallback. Map each Exotel `start.custom_parameters.agent_ref` to an active AGENT identity with exactly one active team membership before accepting calls. The path accepts only a complete connected/start/media/stop lifecycle with contiguous sequence, chunk and timestamp positions, bounded 8 kHz mono PCM, and submits the WAV through ordinary durable intake after stop. Intake locks and rechecks the same mapping and exactly-one active membership through commit. A gap, disconnect, stale generation, changed/revoked mapping, or disabled integration fails closed. Integration audit events reject updates and deletes.

Admission is capped at eight simultaneous connections per API worker, including Basic authentication work. The connected/start handshake has a 10-second total deadline, idle media receives time out after 30 seconds, and a session is limited to 7,230 seconds. Database authentication/lookups use five-second connection and statement timeouts; final fenced intake uses the remaining session budget as its statement timeout. These are resource bounds, not load or availability qualification.

Live inference is limited to one model decode at a time per API process. If a native decoder call never returns, the affected preview degrades and later live previews in that process cannot acquire the decoder gate; recording intake remains independent. Python cannot safely terminate a running native inference thread, so an operator must restart the affected API process to restore live-preview capacity. The admission and executor bounds prevent an unbounded thread backlog, but do not provide decoder OS-level isolation; this remains a qualification gap.

This is code-path evidence from synthetic WebSocket and browser tests, not an Exotel tenant test. Account entitlement, actual callflow configuration, media direction/coverage and production WSS networking remain unverified. Live STT has not been model-qualified on target hardware; supervisor UI displays the redacted provisional preview and degraded/stale state, but no sentiment alerts, two-leg attribution, capacity qualification, vendor operations or real-call processing is established. Use synthetic traffic until real-data and retention approvals are in place.
