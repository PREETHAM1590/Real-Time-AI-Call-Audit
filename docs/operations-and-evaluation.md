...
The advisory `SENTIMENT` post-call worker stage (`app.worker.make_sentiment_processor`) wraps the existing `app.sentiment_adapter.LocalSentimentAdapter`/`app.sentiment.customer_speech_trend`. It is registered only when both `SENTIMENT_MODEL_PATH` (an absolute, read-only local checkpoint directory) and `SENTIMENT_MODEL_SHA256` (that directory's verified recursive digest) are set; an incomplete pair fails worker startup like the other model stages, and an unconfigured pair leaves `SENTIMENT` parked. `SENTIMENT_ADAPTER_VERSION` optionally overrides the recorded adapter version string (default `local-sentiment-v1`). Deployment's worker image must additionally install a `transformers` release and a matching `torch` build for the target hardware; this repository's own `pyproject.toml`/`uv.lock` intentionally do not pin either, so `app.sentiment_adapter.transformers_classifier` raises a clear `RuntimeError` at worker startup (fail closed) if `transformers` is not importable, rather than the stage silently running unconfigured or downloading anything. The classifier sets `HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1` before import, loads only the verified local directory, and never fetches a model by name at runtime.

Output is persisted to `call_sentiments` (migration 021): per-call revisioned, immutable (UPDATE is rejected; DELETE remains allowed so `app.retention.purge_call` can remove it), and scoped to `(organisation_id, call_id, transcript_revision, model_artifact, adapter_version)` so a rerun for the same evidence and model is idempotent. Every persisted signal carries only `id, start_ms, end_ms, signed_score, top_class_probability` — no utterance text is ever stored, logged, or returned by `GET /v1/calls/{id}` (which exposes only a summary: status, trend, alert offsets, failed-utterance count and a signal count). Sentiment is advisory only: `app.ingest.finish_job`'s sentiment branch never changes `calls.processing_state`, so it can never move a call to `READY` or override `NEEDS_REVIEW`. The analyst UI shows a "Sentiment (advisory, uncalibrated)" card with a fixed disclaimer that it is never used to pass or fail a call and that model confidence is not calibrated. As with the rest of this stage's lineage (`docs/open-source-models.md`'s "Sentiment" row), no real checkpoint has been selected, downloaded or evaluated on contact-centre data, and no calibration, latency or throughput evidence exists for this wiring; `tests/test_sentiment_pipeline.py` uses deterministic fakes only.
