...
This is not a production-readiness claim: it is a single Kaggle run, on synthetic silence, that exercises only half the pipeline. Before this row can support a provisioning decision, `run_benchmark.py.template`'s `pip_install` calls need pinned `torch`/`transformers`/`vllm`/CUDA-runtime versions compatible with the Kaggle GPU image (or the `vllm` path dropped in favour of a version-pinned `transformers` fallback only), and the run repeated to get an actual audit-LLM latency/schema/memory measurement.
