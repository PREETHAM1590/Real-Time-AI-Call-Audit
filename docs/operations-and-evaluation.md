...
Exotel WSS recording intake now persists only a hashed call key, integration/generation identifiers, mapped agent/team IDs and UTC lifecycle timestamps. The scoped `/v1/live-calls` snapshot exposes LIVE, DRAINING, ENDED and INCOMPLETE states, including stale LIVE/DRAINING activity. It provides no transcript, media, sentiment or live inference; Exotel account entitlement, callflow configuration and production connectivity remain unverified.
