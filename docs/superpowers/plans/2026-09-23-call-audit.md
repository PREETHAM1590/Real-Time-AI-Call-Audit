...
**Step 5b tooling note:** `app.transcript_verification` (word-level edit-distance WER, missed/invented/misheard words, per-utterance attribution; `tests/test_transcript_verification.py`) exists so this gate can be run the moment approved samples and a human reference transcript are available — it does not itself supply either. It compares whatever hypothesis text it is given against a reference; comparing a raw human reference against a *redacted* hypothesis will show a spurious mismatch at every redacted span, so redact the reference the same way first, or compare against pre-redaction STT output kept only locally and never persisted (see the module's own privacy note). No real recordings, real transcripts or real WER numbers exist here.
...
**Task 7A status:** provider-independent `FinalBuffer` steps 1–3 and the Exotel AgentStream frame/session parser are implemented and checked. The parser follows the published 8 kHz mono PCM-over-base64 event shape, records sequence/chunk gaps and keeps speaker role UNKNOWN; see `app/exotel.py`.

**Task 7B status:** tenant-scoped Exotel integration administration, write-only generated Basic credentials, active agent/team mapping, a WSS recording-intake prototype, bounded PCM spool, database generation fencing and reuse of durable post-call intake are implemented with synthetic checks. The prototype requires HTTP Basic, but public docs do not establish that the unidirectional Stream applet sends Basic credentials; confirm this with an account before enablement. Only clean, gap-free stop commits a recording job. Exotel accounts and callflows are not connected or qualified.

**Task 7C status:** Exotel sessions persist LIVE after authenticated integration, current mapping, active membership and generation validation; clean stop moves to DRAINING before recording intake, accepted intake and ENDED commit atomically, and any post-activation failure/disconnect moves to INCOMPLETE unless a durable-call reconciliation confirms completion. `GET /v1/live-calls` is tenant/team/agent scoped, hides legacy UNKNOWN rows, and recovers LIVE sessions after 90 seconds without activity and DRAINING sessions after 2 hours 1 minute. A tenant/team/agent-authorized read API and 15-minute redacted provisional transcript buffer are fenced to the current session generation and blocked for tombstoned calls; read access continues briefly after ENDED/INCOMPLETE while rows remain within TTL. Purge writes a durable marker to each matching session before call identifiers are erased, preventing reactivation. A dedicated worker thread runs a TTL sweeper every 60 seconds even while a job is active, deleting at most 1,000 rows per pass; at a single worker the cleanup lag is roughly one interval plus one interval per 1,000-row backlog batch. Multiple workers may drain the backlog faster. `EXOTEL_LIVE_TRANSCRIPTION=1` opts the authenticated Exotel PCM path into a bounded 40-block (four-second) queue of 100 ms mono chunks and the local `LiveWindowTranscriber`; it stores only redacted utterances. Model loading, inference, redaction, queue-overflow or transient-storage errors mark the generation DEGRADED and never interrupt the independent durable PCM spool/intake. Transcript state is persisted as DISABLED/EMPTY/LIVE/DEGRADED and the latest 500 utterances are retained with an explicit truncation flag. The analyst page polls and renders authorized redacted provisional utterances, preserves stale session indicators, and states that post-call transcript is authoritative. Early disconnects cancel the preview consumer and discard queued/raw/candidate transcript state. Speakers stay UNKNOWN for mono. The 15-minute preview and synthetic route/UI checks are implemented; model quality/latency/capacity, stable-live finalisation, sentiment, alerts, two-leg attribution and provider account qualification remain incomplete.
- [x] **3A. Add the docs-supported TeleCMI parser slice:** `app/telecmi.py` accepts bounded opaque binary messages, caps per-message bytes, cumulative bytes and elapsed session time, and labels the speaker `UNKNOWN`; `tests/test_telecmi.py` covers these limits. It does not decode, sequence, timestamp, assign channels, authenticate or persist frames. The provider page and specification show this as parser-only and disabled.
- [ ] **3B. Enable TeleCMI intake only after contract confirmation:** obtain an authenticated provider test account and confirm inbound WSS authentication/signature or another verifiable session-binding mechanism; exact binary envelope/framing, sample encoding, rate, channels, timestamps/order and call/session identity; webhook authentication; and caller/callee-to-agent/customer mapping. Enforce message-size, byte/time and connection/concurrency limits at the WebSocket transport before it buffers complete messages. Then add the guarded endpoint and end-to-end tests. The reviewed public docs do not define those contracts, and the outbound PCMO appid/secret is not treated as proof of inbound WSS or webhook authentication.
- [ ] **4. Implement the first verified live-media adapter** after confirming product, account entitlement, authentication and media format. Validate signature/session scope, track IDs, codec, sequence, timestamps, payload length and base64. Bound input queue by bytes and time; 200-ms reorder buffer initially. Reject replayed sessions and mark gaps instead of filling fabricated audio. Send bounded overlapping speech windows from local Silero VAD to local faster-whisper; record window-to-stable-final latency and correction rate. Add post-call recording/event adapters only after confirming their separate APIs and retention/link semantics; qualify each adapter independently.
...
**Step 4 substatus:** `app.sentiment_adapter.LocalSentimentAdapter` is implemented and unit-tested with a deterministic fake classifier: it verifies a local pinned model directory by SHA-256 (same helper as the transcription/audit adapters), sends only final redacted CUSTOMER utterance text, validates the returned class-probability shape and fails closed (never raises past the caller) on malformed output or inference failure, and has a `warm()` method. `compute_call_sentiment` combines it with the existing pure `sentiment_alert_times`, and returns an explicit `UNAVAILABLE`/`UNKNOWN` result rather than blocking when no model is configured or every utterance fails. `app.sentiment.customer_speech_trend` implements the spec's first/last-60-seconds-of-customer-speech call summary with duration weighting (`tests/test_live.py`); a contiguous-turn aggregate is not implemented because the spec does not define a turn boundary and one was not invented here.

This adapter is now wired end to end, but still with no real checkpoint selected: migration 021 adds an immutable-but-deletable `call_sentiments` table (`organisation_id,id` PK; unique per `(organisation_id,call_id,revision)` and per `(organisation_id,call_id,transcript_revision,model_artifact,adapter_version)`; signals limited to `id,start_ms,end_ms,signed_score,top_class_probability`, never text). `app.worker.make_sentiment_processor`/`build_processors` add a `SENTIMENT` stage that loads the call's current final transcript revision, runs `compute_call_sentiment`, computes `customer_speech_trend` when `status="OK"`, and is registered only when both `SENTIMENT_MODEL_PATH` and `SENTIMENT_MODEL_SHA256` are set (and `transformers` is importable in the worker image; see `docs/operations-and-evaluation.md`'s Sentiment stage note) — an unconfigured or partially configured pair leaves the stage parked or fails worker startup, matching the other model stages. `app.ingest.finish_job`'s `"sentiment"` branch enqueues `SENTIMENT` alongside `ANALYSE`/`POLICY` after each transcript commit, re-validates every field (rejecting any signal that carries a key outside the five allowed, including `text`) before persisting, and never changes `calls.processing_state`: sentiment stays advisory only, never a pass/fail input. `app.retention.purge_call` deletes `call_sentiments` rows for a tombstoned call; `app.reviews.call_detail` exposes only a summary (revision, status, trend, alert offsets, failed-utterance count, signal count — never the per-utterance signals) tenant-scoped the same way as the rest of the call; `web/app.js` renders it as a "Sentiment (advisory, uncalibrated)" card next to the disposition card, with a fixed disclaimer line, and states "Not available" when no model is configured. **Still not done, so this box stays unchecked:** no real pinned checkpoint is selected, downloaded or evaluated (`docs/open-source-models.md`'s "Sentiment" row is unchanged), no `transformers`/`torch` runtime has been added to this project's own dependencies (deployment provisions them in the worker image; see the operations guide), and model confidence remains uncalibrated per the operations guide until measured against adjudicated data. Tests: `tests/test_sentiment_pipeline.py` (unit-level processor/build_processors behaviour and PostgreSQL integration for enqueue, persistence, idempotency, staleness, text-leak rejection, immutability-but-deletable, retention purge and tenant scoping) and `tests/test_browser_review.py::test_sentiment_card_renders_trend_and_alert_then_not_available_when_null`.
...
**Step 6 substatus:** the call review page's SSE reconnect is implemented; the active-call cards with policy/sentiment findings this step describes are not. `web/app.js` opens `/v1/events` with `fetch`+`ReadableStream` (matching the cookie-session credentials the page already uses for every other call) when a call is opened in the detail view, tracks `Last-Event-ID`, and shows a real text connection state — "Live", "Reconnecting…", or "Disconnected" — never a color-only cue and never implying a disconnected feed is current; any stream close (network failure or a clean end, which a well-behaved `/v1/events` never does on its own) is treated as a disconnect and retried with a doubling backoff, capped, and reset on the next successful connect. A `call.updated` event for the open call refreshes only `processing_state`/`transcript_revision` in place, de-duplicated by sequence, so review-in-progress form state is preserved rather than discarded by a full reload; a `reset_required` event shows "Feed reset, reloading…" and refetches the call. A 401/403 is treated as terminal (shown as "Disconnected", not retried) matching how `loadLiveCalls` already treats the same statuses. `tests/test_browser_review.py::test_call_review_sse_reconnects_and_applies_exactly_one_in_place_update` disconnects the feed, asserts "Reconnecting…", reconnects with the previously received cursor as `Last-Event-ID`, and asserts exactly one in-place update (no duplicate summary card, no leftover stale value) after the replayed event — matching this step's browser-test description. `tests/test_events.py::test_cross_org_cursor_values_neither_leak_nor_confuse_organisation_bs_own_stream` covers this step's backend-test description: an organisation B scope with a cursor value that is valid (and even the latest) within organisation A's own numbering is evaluated against B's own stream and raises `EventCursorError` rather than returning any of A's rows, and B's own cursor-0 read never contains A's call id or payload. Separately, the existing "Live call sessions" panel (`web/index.html`/`web/app.js`, `#live-calls-list`) now states "Live sentiment: not available (post-call only)" and "Policy alerts: not evaluated live" on every card, so the panel cannot be read as claiming either capability. No textual policy/sentiment finding, evidence link, acknowledgement control, or supervisor card is built by this slice; this step's box stays unchecked.
...
**Task 8 status:** provider-independent Steps 1–3 and Step 5's post-call state feed are implemented, plus the reconnect/cursor-resume and honest-capability-disclosure slice of Step 6 described above (browser and backend tests included). Step 4's local sentiment adapter is now wired into an advisory post-call `SENTIMENT` worker stage, persistence, the call-detail API and the analyst UI per its own substatus above, but no real checkpoint has been selected, downloaded or evaluated, so Step 4's box stays unchecked. No live media/policy event workflow or evaluated live findings exist, no active-call cards with policy/sentiment evidence or acknowledgement are built, and the disposition re-run-after-live-finalisation step remains unimplemented. Latency was not measured for this slice; only the deterministic browser tests above were run, so no latency number is claimed.
...
- [ ] **4b. Complete observability:** a metrics exporter/time-series store, alerting, per-stage dashboards and spend/capacity telemetry are still required. No model-readiness or GPU/compute readiness claim is implemented.
  - Substatus (2026-09-25): `GET /metrics` (Prometheus text 0.0.4, separate bearer-token auth, `app/metrics.py`) now exports aggregate, no-identifier gauges/counters for job/call/stage/live-session state from existing tables plus a new cross-process `worker_stage_counters` table (migration `022_worker_stage_counters.sql`) the worker upserts per claimed attempt; it degrades to `call_audit_database_up 0` rather than 500ing. `deploy/prometheus/alerts.yml` + `deploy/prometheus/README.md` give a proposed, undeployed rule file and scrape config built only from those series. `operations_summary` gained a tenant-scoped `pending_by_stage` breakdown, rendered as a small table on `/dashboard`. Still missing, so the box stays unchecked: no Prometheus/Alertmanager deployment or wiring anywhere in this repo, no alert delivery/notification of any kind, no time-series storage, and no GPU/token/spend or redaction-failure/STT-failure-rate telemetry (today's per-stage/per-outcome counters cannot isolate those specific causes).
