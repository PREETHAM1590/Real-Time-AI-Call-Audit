...
**Task 7B status:** tenant-scoped Exotel integration administration, write-only generated Basic credentials, active agent/team mapping, authenticated WSS recording intake, bounded PCM spool, database generation fencing and reuse of durable post-call intake are implemented with synthetic checks. Only clean, gap-free stop commits a recording job. Exotel accounts and callflows are not connected or qualified.

**Task 7C status:** Exotel sessions persist LIVE after authenticated integration, current mapping, active membership and generation validation; clean stop moves to DRAINING before recording intake, accepted intake and ENDED commit atomically, and any post-activation failure/disconnect moves to INCOMPLETE unless a durable-call reconciliation confirms completion. `GET /v1/live-calls` is tenant/team/agent scoped, hides legacy UNKNOWN rows, and recovers LIVE sessions after 90 seconds without activity and DRAINING sessions after 2 hours 1 minute. A tenant/team/agent-authorized read API and 15-minute redacted provisional transcript buffer now exist, fenced to the current session generation and blocked for tombstoned calls; purge writes a durable marker to each matching session before call identifiers are erased, preventing reactivation. Buffer writes are not connected to inference or Exotel, so live utterances are not yet produced or shown in the UI. A dedicated thread in the worker runs a TTL sweeper every 60 seconds even while a job is active, deleting at most 1,000 rows per pass; at a single worker the cleanup lag is roughly one interval plus one interval per 1,000-row backlog batch. Multiple workers may drain the backlog faster. The analyst page shows lifecycle and stale state. A deterministic `LiveWindowTranscriber` prototype now handles bounded overlapping 8 kHz PCM windows through the local Faster-Whisper API shape, emits only redacted provisional/correction/final utterances, and keeps speakers UNKNOWN. It is not yet wired to Exotel: no producer writes inference results to the buffer, and the browser has no live utterance feed. Live transcription remains unavailable in the product until ingestion and supervisor delivery are connected. Sentiment, alerts, two-leg attribution and account qualification are also incomplete.
