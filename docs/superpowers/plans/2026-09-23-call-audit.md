...
**Task 7C status:** Exotel sessions persist LIVE after authenticated integration, current mapping, active membership and generation validation; clean stop moves to DRAINING before recording intake, accepted intake and ENDED commit atomically, and any post-activation failure/disconnect moves to INCOMPLETE unless a durable-call reconciliation confirms completion. `GET /v1/live-calls` is tenant/team/agent scoped, hides legacy UNKNOWN rows, and recovers LIVE sessions after 90 seconds without activity and DRAINING sessions after 2 hours 1 minute. The analyst page shows lifecycle and stale state. This is session visibility only: live transcription, sentiment, alerts, two-leg attribution and account qualification are not implemented.
