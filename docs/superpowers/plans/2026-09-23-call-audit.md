...
**Verification evidence:** on 2026-09-23, `python -m compileall -q app tests`, `python -m app.evaluate --dataset tests/fixtures/golden.jsonl --output <temporary report>`, `git diff --check`, and `python -m unittest discover -s tests -v` passed; the latest full run was 125/125 using a newly created `call_audit_task10audit_test` PostgreSQL database with `RUN_POSTGRES_INTEGRATION=1`. The retention regression includes review and audio-grant event lineage. The report is a synthetic fixture smoke test only; its counts are not model-quality measurements. No restore, load, pinned-model, CI/container or production-deletion qualification ran.
