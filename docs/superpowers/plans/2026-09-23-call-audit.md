...
**Task 7C status:** Exotel sessions persist LIVE after authenticated integration, current mapping, active membership and generation validation; clean stop moves to DRAINING before recording intake, accepted intake and ENDED commit atomically, and any post-activation failure/disconnect moves to INCOMPLETE unless a durable-call reconciliation confirms completion. `GET /v1/live-calls` is tenant/team/agent scoped, hides legacy UNKNOWN rows, and recovers LIVE sessions after 90 seconds without activity and DRAINING sessions after 2 hours 1 minute. The analyst page shows lifecycle and stale state. A deterministic `LiveWindowTranscriber` prototype now handles bounded overlapping 8 kHz PCM windows through the local Faster-Whisper API shape, emits only redacted provisional/correction/final utterances, and keeps speakers UNKNOWN. It is not yet wired to Exotel because the application has no scoped ephemeral transcript consumer; running inference and discarding results would waste local capacity. Live transcription remains unavailable in the product until a tenant-scoped ephemeral event/session contract exists. Sentiment, alerts, two-leg attribution and account qualification are also incomplete.
