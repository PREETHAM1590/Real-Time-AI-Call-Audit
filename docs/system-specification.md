...
|---|---|---|
| Exotel AgentStream / Programmable Voice | Official docs describe the Stream applet's WSS URL and connected/start/media/stop lifecycle, custom parameters and 8 kHz mono PCM-over-base64: [Stream applet](https://docs.exotel.com/exotel-agentstream/stream-applet), [StreamKit start example](https://docs.exotel.com/exotel-agentstream/streamkit-cloud). The support guide describes Basic authentication under the bidirectional Voicebot applet, but does not establish it for the unidirectional Stream applet: [WSS authentication guide](https://support.exotel.com/support/solutions/articles/3000108630-working-with-the-stream-and-voicebot-applet) | The synthetic-tested adapter currently requires tenant-scoped HTTP Basic credentials and implements ADMIN mapping plus a gap-rejecting 8 kHz mono parser. Compatibility of Basic headers with Exotel's unidirectional Stream applet is unverified; do not enable against a tenant until its handshake/authentication contract is confirmed. Account entitlement, callflow behavior, agent-leg coverage and role attribution also remain unverified; mono audio stays UNKNOWN |
| TeleCMI PIOPIY AI Streaming | Official docs describe WebSocket audio streaming, `listen_mode` caller/callee/both and `voice_quality` 8000/16000; the example URL uses `ws://`: [AI Streaming](https://doc.telecmi.com/piopiy/docs/ai-streaming/). Outbound PCMO uses app ID and secret: [Outbound AI Streaming](https://doc.telecmi.com/piopiy/docs/outbound-ai-streaming/). Webhook docs list stream lifecycle events: [Webhooks overview](https://doc.telecmi.com/piopiy/docs/webhooks-overview/) | A bounded opaque-binary parser helper exists, but no WebSocket endpoint or recording intake is enabled. Public docs reviewed do not establish transport security or inbound WebSocket authentication/signatures, binary framing, sample encoding, timestamps, sequence numbers or channel IDs. Speaker role remains UNKNOWN. |
...
| KooKoo Voice | Official product page claims bidirectional PCMU WebSocket streams at 8 kHz and a NewCall → Stream → Hangup lifecycle: [KooKoo voice platform](https://www.kookoo.in/) | Live media capability stated; detailed authenticated wire contract, plan entitlement, account behavior and role mapping remain unverified |
...
| MyOperator | Webhook v2 supports HTTPS POST with Basic or API-key authentication (including header keys), `call.end` and `call.summary`, `event_id` deduplication, and automatic retries up to 24 hours/26 attempts: [setup and retry guide](https://support.myoperator.com/portal/en/kb/articles/how-do-i-configure-and-manage-webhook-in-myoperator), [payload schema](https://support.myoperator.com/portal/en/kb/articles/what-is-the-exact-payload-structure-for-every-myoperator-webhook-v2-event). Legacy recording API accepts its token in the query string and says links expire after 24 hours: [API index](https://signup.myoperator.com/api/). | Webhook contract is documented; no adapter is enabled. Recording API response, safe token transport, download authentication/redirects, webhook filename mapping and tenant entitlement are unverified. Do not fetch recordings until qualified. |
...
| CloudConnect | Official [API reference](https://api.cloud-connect.in/) documents HTTPS JSON APIs and authentication using a user login plus API token or password. Its [CallLog guide](https://api.cloud-connect.in/CCCPL-CallLog%20API_v1.0.pdf) documents a `call_rec_path` field, with an example containing `-` | Call-log metadata documented; recording availability, URL retrieval/authentication/expiry and live media remain unverified |

These rows are research inputs, not an implementation promise. Recheck current vendor docs and account entitlements at adapter implementation. A recording URL, active-call list, call-event webhook or recording pause/resume operation is not a live audio stream. The UI must distinguish `Live audio`, `Post-call recording`, and `Call events` and show `Protocol/entitlement unverified` where appropriate. Adapter configuration is tenant-scoped, secrets are write-only and never returned to the browser, and self-hosted STT/LLM processing remains unchanged. For TeleCMI, the binary parser is not an enabled integration: obtain vendor confirmation of transport security, authenticated stream setup, binary message/audio format, timing/order semantics and channel-to-party meaning before connecting it to a tenant call or durable intake.
...
The current Exotel preview is opt-in with `EXOTEL_LIVE_TRANSCRIPTION=1`, uses only the configured pinned local Faster-Whisper artifact, and remains separate from the durable audio spool. Each API worker bounds queued live audio to 40 fixed 100 ms PCM blocks; overload, local inference, redaction or preview-storage failure degrades the preview while the complete validated recording continues to durable intake. Mono speaker role stays `UNKNOWN`. The tenant/generation-scoped status is `DISABLED`, `EMPTY`, `LIVE` (a redacted preview was stored), or `DEGRADED`; older rows may be evicted after 500 utterances and the preview is marked truncated. The 15-minute preview can be read during the session and shortly after its end, subject to normal scope and tombstone checks. It never supplies post-call audit evidence. Stable-live finalisation and its DRAINING barrier remain a later task.
