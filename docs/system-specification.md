...
| MyOperator | Official [API index](https://signup.myoperator.com/api/) documents post-call recording-link retrieval with 24-hour validity; its legacy API passes the token in the query string. A newer [support guide](https://support.myoperator.com/portal/en/kb/articles/what-are-apis-23-8-2025) describes Bearer authentication, Push/Pull modes and account rate limits. | Post-call recording/events documented; select the account's contracted API version and authentication mode before implementation. Link access scope and observed expiry still require account qualification. |
