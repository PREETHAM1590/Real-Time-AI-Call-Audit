...
class QualityReportBrowserTests(unittest.TestCase):
    ...
    def test_provider_route_navigation_and_capability_matrix_are_explicit(self):
        ...
        with self.run_report_page(handle) as page:
            ...
            telecmi = table.get_by_role("row", name=re.compile("TeleCMI"))
            self.assertIn("transport security and authentication are unverified", telecmi.inner_text())
            cloudconnect = table.get_by_role("row", name=re.compile("CloudConnect"))
            self.assertIn('example is “-”', cloudconnect.inner_text())
            self.assertIn("no event or webhook delivery contract was found", cloudconnect.inner_text())
            self.assertEqual(cloudconnect.locator("td").nth(2).locator(".capability").inner_text(), "Unverified")
            kookoo = table.get_by_role("row", name=re.compile("KooKoo"))
            self.assertIn("detailed authenticated protocol", kookoo.inner_text())
            self.assertTrue(table.get_by_text("Other / custom integration").count())
            self.assertTrue(table.get_by_text("agent-leg coverage and role attribution remain unverified.").count())
