...
class LiveSessionLifecycleTests(unittest.TestCase):
    ...
    def test_migration_hides_legacy_sessions_as_unknown_and_backfills_times(self):
        ...
        status_migration = (Path(__file__).resolve().parent.parent / "migrations" / "020_live_transcription_status.sql").read_text(encoding="utf-8")
        self.assertIn("'DISABLED','EMPTY','LIVE','DEGRADED'", status_migration)
        self.assertIn("live_transcript_truncated", status_migration)
